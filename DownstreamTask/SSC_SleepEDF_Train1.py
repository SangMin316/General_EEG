...
tempPath = torch.load('/home/wypark/smj/Temporal_pathway/model/weak/10s/0.76Temporal_10s_ep4_.pt')


# model
model = TIPNet.StatisticianPipe1(dense = 256*2, channels=2, classes= 5).to(device)
# model = TIPNet.StatisticianPipe5(dense = 256, channels=2, classes= 5).to(device)
#
...
for epoch in range(epochs):
    ...
    torch.save(model, '/home/wypark/smj/SSC_SleepEDF/model/ConvDense/freezing/' + 'SleepEDF_DIP_ep' + str(epoch) + '.pt')
...
df.to_excel('/home/wypark/smj/SSC_SleepEDF/result/ConvDense/freezing/'+'SleepEDF.xlsx', index = False)
...
plt.savefig('/home/wypark/smj/SSC_SleepEDF/result/ConvDense/freezing/'+'SleepEDF_loss',bbox_inches = 'tight')
plt.show()
...
plt.savefig('/home/wypark/smj/SSC_SleepEDF/result/ConvDense/freezing/'+'SleepEDF_accuracy',bbox_inches = 'tight')
plt.show()