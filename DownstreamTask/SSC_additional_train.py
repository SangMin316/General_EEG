...
#dataloader
train, test = train_test_split(SleepEDF_list, test_size=0.2, random_state= 7)
...
tempPath = torch.load('/home/wypark/smj/Temporal_pathway/model/10s/Temporal_10s_ep2_.pt')

# model
ep_n = 50
epochs = epochs + ep_n
model = torch.load('/home/wypark/smj/SSC_SleepEDF/model/Dense/SleepEDF_DIP_ep'+str(ep_n-1)+'.pt').to(device)

# loss, optimizer
normedWeights = WCE_weight().to(device)
...
for epoch in range(ep_n,epochs):
    ...
    torch.save(model, '/home/wypark/smj/SSC_SleepEDF/model/Dense/' + 'SleepEDF_DIP_ep' + str(epoch) + '.pt')
