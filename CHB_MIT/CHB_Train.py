...
import TIPNet
from CHB_MIT.CHB_MIT_WCE import WCE_weight


# load a data adress
CHBMIT_file_list = sorted(glob.glob('/DataCommon2/wypark/smj_data/preprocessedCHB_MIT/**'))
print(len(CHBMIT_file_list))
# we use one subject as testdataset
...
for i in range(len(CHB_mit)):
    if i == exclude_n:
        test_subject = str(CHBMIT_file_list[i]).split('/')[-1]
        print('excluded subject:', test_subject)
        continue
...
#dataloader
train, val = train_test_split(CHB_LIST, test_size=0.2, random_state= 770)

train_dataset = CHB_MIT_dataset(train,200,SSL = False)
val_dataset = CHB_MIT_dataset(val,200,SSL = False)
...
model = TIPNet.StatisticianPipe1(dense = 256*2, channels=23, classes= 2).to(device)
# model = TIPNet.StatisticianPipe0(dense = 256, channels=2, classes= 5).to(device)


# model = TIPNet.StatisticianPipe2(dense = 256, channels=2, classes= 5).to(device)
# model = TIPNet.StatisticianPipe5(dense = 256, channels=23, classes= 2).to(device)
...
for epoch in range(epochs):
    ...
    # ad = '/home/wypark/smj/DonstreamTask_CHBMIT/model/DIP_Conv/2s/' + test_subject + '/' + str(acc_val[epoch])[:5] + '_' + str(epoch)
...
df.to_excel('/home/wypark/smj/DonstreamTask_CHBMIT/result/DIP_Conv/2s/'+'SleepEDF.xlsx', index = False)
...
# plt.savefig('/home/wypark/smj/DonstreamTask_CHBMIT/result/DIP_Conv/2s/'+'SleepEDF_loss',bbox_inches = 'tight')
plt.show()
...
# plt.savefig('/home/wypark/smj/DonstreamTask_CHBMIT/result/DIP_Conv/2s/'+'SleepEDF_accuracy',bbox_inches = 'tight')
plt.show()
